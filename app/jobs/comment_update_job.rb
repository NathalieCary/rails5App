class CommentUpdateJob < ApplicationJob
  ...
  def perform(comment, current_user)
	  ProductChannel.broadcast_to(comment.product_id, comment: render_comment(comment, current_user), average_rating: comment.product.average_rating, highest_rating_comment: comment.product.highest_rating)
	end
